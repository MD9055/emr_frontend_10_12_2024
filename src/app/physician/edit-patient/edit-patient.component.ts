...
export class EditPatientComponent implements OnInit {
  ...
  onCancel() {
    this.router.navigateByUrl('/physician/patient')
   }
   updatePatient() {
    ...
    const updatedPatientData = {
      ...
      kin_information: {
        firstName: this.patientForm.value.kin_first_name,
        lastName: this.patientForm.value.kin_last_name,
        kin_address_street1: this.patientForm.value.kin_address_street1,
        kin_address_street2: this.patientForm.value.kin_address_street2,
        zip_code: this.patientForm.value.kin_zip_code,
        cell_number: this.patientForm.value.kin_cell_number,
        landline_number: this.patientForm.value.kin_landline_number,
        country: this.patientForm.value.kin_country,
        state: this.patientForm.value.kin_state,
        city: this.patientForm.value.kin_city,
      }
    };
  
    this.commonService.post(`physician/updatePatient?_id=${this.adminId}`, updatedPatientData).subscribe(
      (response: any) => {
        if (response.statusCode === 200) {
          this.toastrService.success('Patient information updated successfully.');
          this.router.navigateByUrl('/physician/patient');
        } else {
      ...
      (error) => {
        ...
        this.toastrService.error('An error occurred while updating patient information.');
      }
    );
  }
